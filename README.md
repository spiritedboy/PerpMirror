...
- 如果程序管理的 Follower 仓位在 Leader 仍持仓时被手动平仓、强平或从交易所快照中消失，程序会将该币种暂停到本轮 Leader 平仓，不会重新补仓；首次检测时发送“跟单暂停”飞书卡片。
- Leader 完全平仓后自动解除上述暂停；只有 Leader 之后再次新开仓，程序才重新跟随该币种。
...
旧版 version 1 所有权文件会在 LIVE 启动时自动迁移到 version 2，用于持久化“仓位已观察到”和“等待 Leader 平仓”的状态；不需要、也不应手工删除状态文件。
